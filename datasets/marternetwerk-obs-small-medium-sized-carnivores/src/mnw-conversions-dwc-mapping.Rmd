...
## Read MNW data exported from SQL database  
Based on R script internally shared at the inbo github in the fis-carnivoren folder 'Marternetwerk' output: 
[Marternetwerk-observations-sql-export-in-csv.Rmd](https://github.com/inbo/fis-carnivoren/tree/main/Projecten/Marternetwerk/Output/Marternetwerk-observations-sql-export-in-csv.Rmd)
```{r}
...
This dataset currently contains a total of `r nrow(ds_mnw_all_obs)` relevant records, representing `r length(unique(ds_mnw_all_obs$scientificName[ds_mnw_all_obs$taxonRank == "species"]))` unique species.
...
### Deleting record(s) with obviously inaccurate point localities
...
### Recheck this subdataset version
...
# Create occurrence.csv files with completed dwc-mapping 

## Create HIGH RESOLUTION (highres) occurrence.csv for restricted access in VBP
```{r}
...
write_csv(ds_mnw_vbp_hires,"ds_mnw_vbp_highres.csv",na = "")
```

Find 'ds_mnw_vbp_highres.csv' file in working directory on pc to upload and publish in INBO devIPT


## Create LOW RESOLUTION (lowres) occurrence.csv for public access in GBIF
```{r}
...
sds_mnw_all_recs_1km <- ds_mnw_vbp_s %>% 
...
sds_mnw_badger_5km <- ds_mnw_vbp_s %>% filter(vernacularName %in% c("Das"))
...
sds_mnw_sens_recs_without_geocoords <- ds_mnw_vbp_s %>% 
...
sds_mnw_no_sen_recs <- ds_mnw_vbp_s %>% 
  filter(!(vernacularName %in% c("Boommarter","Otter","Wilde kat","Das") | WaarnemingType %in% c("Opname fotoval"))) %>% 
  mutate(dataGeneralizations = as.character(NA)) %>% 
  mutate(informationWithheld = as.character(NA))
...
write_csv(ds_mnw_all,"ds_mnw_lowres.csv",na = "") 
```

Find 'ds_mnw_lowres.csv' file in working directory on pc to upload, register and publish in INBO IPT

### Preview this dataframe version with data generalisations
```{r}
#head(occurrence)
sds_mnw_all_lr_geo <- ds_mnw_all %>% filter(geodeticDatum %in% "WGS84") 
...
head(occurrenceMap, n = 30)
```

### Check this dataset version based on mapview
```{r}
